...
GPXParser.prototype.addTrackSegmentToMap = function(trackSegment, colour, width) {
	...
	for(var i = 1; i < trackpoints.length; i++) {
		...
		distanceCovered += calculateDistance(trackpoints[i - 1], trackpoints[i]);
	}
	...
	return parseFloat(distanceCovered / 1000).toFixed(2);
}
...
GPXParser.prototype.addTrackToMap = function(track, colour, width) {
	...
	for(var i = 0; i < segments.length; i++) {
		totalDistanceCovered += parseFloat(this.addTrackSegmentToMap(segments[i], colour, width));
	}
...
GPXParser.prototype.addTrackpointsToMap = function() {
	var distanceCoveredOnTrack = 0;
	var tracks = this.xmlDoc.documentElement.getElementsByTagName("trk");
	for(var i = 0; i < tracks.length; i++) {
		distanceCoveredOnTrack += this.addTrackToMap(tracks[i], this.trackcolour, this.trackwidth);
	}
	return distanceCoveredOnTrack;
}
...
function calculateDistance(prevPoint, currPoint) {
	var R = 6371e3; // km
	var φ1 = toRadians(prevPoint.getAttribute("lat"));
	var φ2 = toRadians(currPoint.getAttribute("lat"));
	var Δφ = toRadians((currPoint.getAttribute("lat") - prevPoint.getAttribute("lat")));
	var Δλ = toRadians((currPoint.getAttribute("lon") - prevPoint.getAttribute("lon")));
